...
すると、新しくパッチ済みの NDS ファイルが生成されます。

<img width="217" height="50" alt="image" src="https://github.com/user-attachments/assets/ab8aeff1-8b1e-4c2a-8ac1-f4e67221abdc" />

これにより、DNS 設定を変更せずに WFC に接続できるようになります。  
※注意：手動で設定した DNS がある場合は必ず削除してください。  
「DNS を自動取得する」を「はい」に設定してください。

<img width="256" height="382" alt="image" src="https://github.com/user-attachments/assets/eeaf8729-03fe-4f9e-9ba3-3a7c66130369" />

この方法を使えば、問題の原因が ISP（インターネットプロバイダ）側にあったとしても、WFC に接続できるようになります。

なお、これを動作させるには **wfcPatcher.exe のバージョン 1.6** を使用する必要があります。  
`.bat` ファイルは `.exe` と同じディレクトリに配置してください。
